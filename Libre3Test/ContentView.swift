...
struct ContentView: View {
            ...
            if let g = manager.glucoseReading {
                ...
                VStack(spacing: 8) {
                    Image(systemName: "drop.fill")
                        .font(.system(size: 40))
                        .foregroundStyle(stateColor.opacity(0.5))
                    Text("Kein Messwert")
                        .font(.headline)
                        .foregroundStyle(.secondary)
    ...
    private var kdfCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            Label("KDF Test Ergebnisse", systemImage: "key.fill")
                .font(.headline)
                .foregroundStyle(.secondary)

            ForEach(manager.kdfResults, id: \.variant.rawValue) { r in
                HStack(spacing: 10) {
                    Image(systemName: r.r2Matches && r.r1Matches
                          ? "checkmark.seal.fill"
                          : r.decrypted != nil ? "exclamationmark.triangle.fill" : "xmark.seal.fill")
                        .foregroundStyle(r.r2Matches && r.r1Matches ? .green : r.decrypted != nil ? .orange : .red)
                        .font(.title3)

                    VStack(alignment: .leading, spacing: 2) {
                        Text(r.variant.rawValue)
                            .font(.caption.bold())
                        if r.r2Matches && r.r1Matches {
                            Text("KORREKT — Handshake erfolgreich!")
                                .font(.caption)
                                .foregroundStyle(.green)
                        } else if r.decrypted != nil {
                            Text("Tag OK, r1/r2 falsch")
                                .font(.caption)
    ...
    private func lineColor(_ line: String) -> Color {
        ...
        if line.contains("🔑") { return .yellow }
        return .primary
    }
