 ...
 *
 *  This file is based on CipherFn.swift from LibreCRKit
 *  (https://github.com/airedev326/LibreCRKit), Copyright (c) 2026 LibreCRKit
 ...
 *
 *  ─────────────────────────────────────────────────────────────────────────
 *  White-box AES VM ("CipherFn / aes_K"), ported clean-room from
 *  LibreCRKit's cipher_fn_v3.py.  13-opcode bytecode VM driven by ~3 MB of
 *  bundled lookup tables (RuntimeTables/).  The "key" is embedded in the
 *  tables — there is no separate key input.
 *
 *  API:
 *    CipherFn.aes_K(input)  →  16-byte white-box AES output
 */
